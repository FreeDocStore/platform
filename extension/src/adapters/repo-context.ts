...
// the shared team memory (.docs-chat/MEMORY.md) and the recent-activity log.
//
...
export const MEMORY_PATH = ".docs-chat/MEMORY.md";
...
/**
 * Read .docs-chat/MEMORY.md from the repo. Returns null when the file
...
export function formatMemoryBlock(content: string | null): string {
  ...
  const capped = lines.length > MEMORY_CAP_BYTES
    ...
    : lines;
  return [
    "Shared team memory for this repo (.docs-chat/MEMORY.md):",
    capped,
    "",
  ].join("\n");
}
