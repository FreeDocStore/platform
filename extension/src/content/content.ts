...
declare global {
  interface Window {
    __glassdocsInjected?: boolean;
  }
}
const alreadyInjected = window.__glassdocsInjected === true;
window.__glassdocsInjected = true;
...
async function getPageContext(): Promise<PageContext> {
  ...
  clone.querySelectorAll("script, style, nav, #glassdocs-edit-btn").forEach((n) => n.remove());
